...
1. Для запуска приложения необходимо перейти в каталог с main.go файлом и запустить его:

```docker-compose up -d```
```cd cmd && go run main.go```

2. Были реализованы слудующие методы:

| Метод | путь |
| --- | --- |
|post| /segments| создание сегмента|
|delete| /segments/id| удаление сегмента|
|post| /segments/user| создание пользователя в сегмент|
|put| /segments/user/id| редактирование данных о пользователях|
|get| /segments/user/id| получение активных сегментов|

3. Исходя из задания мне стало понятно, что необходиомо было реализовать БД из 3 таблиц: сегменты, пользователи и соединительная таблица для реализации связи М:М

4. Также было реализовано дополнительное задание по добавлению пользователю TTL. Было решено задать его датой, а само механизм удаления переложить на триггер в спроектированной БД, а также была написана функция валидации даты для пользователя(которая пока не используется)

5. Для обеспечения сохранности данных я прибег к использованию механизма транзакций.

6. Сгенерирована swagger документация.

7. Также была сделана сущность для хранения истории, которую я планировал хранить в БД, заполняя посредством триггера при заполнении таблицы Пользовательских данных, но реализовать не успел. Для меня остался вопрос открытым о том как заполнять данные о типах операций, когда приходит список сегментов на удаление и на добавление одновременно.
